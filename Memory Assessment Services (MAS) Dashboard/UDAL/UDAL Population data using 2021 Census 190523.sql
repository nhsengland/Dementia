...
IF OBJECT_ID ('[MHDInternal].[TEMP_DEM_MAS_Population_Age_Base_Table]') IS NOT NULL DROP TABLE [MHDInternal].[TEMP_DEM_MAS_Population_Age_Base_Table]
SELECT DISTINCT
	l.MSOA21
	...
	,a.Measure_Name
    ,a.[Count]
INTO [MHDInternal].[TEMP_DEM_MAS_Population_Age_Base_Table]
...
IF OBJECT_ID ('[MHDInternal].[DASHBOARD_DEM_MAS_PopsData]') IS NOT NULL DROP TABLE [MHDInternal].[DASHBOARD_DEM_MAS_PopsData]
SELECT
	Org_Type
	,Org_Name
	,Org_Code
	...
	,Pop
INTO [MHDInternal].[DASHBOARD_DEM_MAS_PopsData]
FROM
(--Sub-ICB
	SELECT
		'Sub-ICB' AS [Org_Type]
		...
		,CASE WHEN [Measure_Name] IN ('Aged 4 years and under','Aged 5 to 9 years','Aged 10 to 15 years','Aged 16 to 19 years','Aged 20 to 24 years' 
			...
			END AS Variable
		,SUM([Count]) AS [Pop]
	FROM [MHDInternal].[TEMP_DEM_MAS_Population_Age_Base_Table]
	GROUP BY 
		[SubICBName], [SubICBCode],
		CASE WHEN [Measure_Name] IN ('Aged 4 years and under','Aged 5 to 9 years','Aged 10 to 15 years','Aged 16 to 19 years','Aged 20 to 24 years' 
									,'Aged 25 to 34 years','Aged 35 to 49 years','Aged 50 to 64 years') THEN 'Under65' 
			WHEN [Measure_Name]='Aged 65 to 74 years' THEN '65to74'
			WHEN [Measure_Name]='Aged 75 to 84 years' THEN '75to84'
...
IF OBJECT_ID ('[MHDInternal].[TEMP_DEM_MAS_Population_Gender_Base_Table]') IS NOT NULL DROP TABLE [MHDInternal].[TEMP_DEM_MAS_Population_Gender_Base_Table]
SELECT DISTINCT
	l.LSOA21
	,l.ODS_SubICB_Code22 AS SubICBCode
	,r.Organisation_Name AS SubICBName
	...
	,g.[Sex]
    ,g.[Count]
INTO [MHDInternal].[TEMP_DEM_MAS_Population_Gender_Base_Table]
...
INSERT INTO [MHDInternal].[DASHBOARD_DEM_MAS_PopsData]
SELECT 
*
FROM
(--Sub-ICB
	SELECT
		...
		,CASE WHEN [Sex]='Female' THEN 'Females'
			...
			END AS Variable
		,SUM([Count]) AS [Pop]
	FROM [MHDInternal].[TEMP_DEM_MAS_Population_Gender_Base_Table]
	GROUP BY [SubICBName], SubICBCode
			,CASE WHEN [Sex]='Female' THEN 'Females'
				WHEN [Sex]='Male' THEN 'Males'
				ELSE 'Other/ Not Stated/ Not Known'
				END
...
IF OBJECT_ID ('[MHDInternal].[TEMP_DEM_MAS_Population_Ethnicity_Base_Table]') IS NOT NULL DROP TABLE [MHDInternal].[TEMP_DEM_MAS_Population_Ethnicity_Base_Table]
SELECT DISTINCT
	l.LSOA21
	,l.ODS_SubICB_Code22 AS SubICBCode
	,r.Organisation_Name AS SubICBName
	...
	,e.Measure
    ,e.Measure_Value
INTO [MHDInternal].[TEMP_DEM_MAS_Population_Ethnicity_Base_Table]
...
(--Sub-ICB
	...
	FROM [MHDInternal].[TEMP_DEM_MAS_Population_Ethnicity_Base_Table]
	GROUP BY [SubICBName], SubICBCode
			,CASE WHEN Measure like '%Asian%' THEN 'Asian'
				WHEN Measure  like'%Black%' THEN 'Black'
				WHEN Measure like'%Mixed%' THEN 'Mixed'
...
DROP TABLE [MHDInternal].[TEMP_DEM_MAS_Population_Age_Base_Table]
DROP TABLE [MHDInternal].[TEMP_DEM_MAS_Population_Gender_Base_Table]
DROP TABLE [MHDInternal].[TEMP_DEM_MAS_Population_Ethnicity_Base_Table]
